...
- **O cabeçalho mostra quem entrou, e "Sair" é um `<form>` com Server Action.** Quem lê a
  ...
  **Desde a RF11 o nome é um link para `/minha-conta`.** O menu ganhou o segundo caminho no
  pedido V1, e desde 03/09/2026 são TRÊS os itens condicionais
  (`compartilhado/itens-de-quem-entrou.ts`): Minha conta para quem tem sessão, **Mural de
  avisos para quem é voluntário ATIVO** e Painel da equipe para quem é equipe — nessa ordem,
  do que alcança mais gente para o que alcança menos, porque a gaveta é lida de cima para
  baixo num celular. Eles NÃO estão em `ITENS`, que continua com os 11 de toda visita: um
  item condicional ali faria `links-menu.test.mjs` e a contagem de 11 medirem um número que
  muda conforme quem olha. **Nada ali autoriza coisa alguma** — o booleano decide o que
  DESENHAR; quem decide o que pode ser lido é a RLS.
...
| RF18 | E-mail de confirmação | **pronto** (03/09/2026) — a Edge Function lê a inscrição pelo par (evento, e-mail), monta a mensagem e manda pelo Resend. MEDIDO contra produção: `{"ok":true}`, linha `enviado` em `public.envios`, e a segunda chamada respondendo `ja_enviado: true` sem mandar de novo. **O que NÃO está provado é a última perna** — o e-mail chegando numa caixa de entrada: o único endereço inscrito hoje é `teste-rf18@exemplo.invalid`, TLD reservado que existe para não chegar em lugar nenhum. O Resend aceitou (200) e a mensagem vai quicar |
